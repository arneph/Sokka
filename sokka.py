...
def TranslateProgram(src: str) -> str:
    ...
    out += '\n'
    out += 'char* itoa(int x) {\n'
    out += '  char* a = malloc(20);\n'
    out += '  sprintf(a, "%i", x);\n'
    out += '  return a;\n'
    out += '}\n'
    out += '\n'
    out += 'char* read(const char* path) {\n'
    ...
    out += '  int length = ftell(f);\n'
    out += '  char* text = malloc(length);\n'
    out += '  fseek(f, 0, SEEK_SET);\n'
    out += '  fread(text, 1, length, f);\n'
    out += '  fclose(f);\n'
    out += '  return text;\n'
    out += '}\n'
    out += '\n'
    out += 'void write(const char* path, const char* text) {\n'
    out += '  FILE *f = fopen(path, "w");\n'
    out += '  if (f == NULL) {\n'
    out += '    printf("could not open file: %s\\n", path);\n'
    out += '    exit(1);\n'
    out += '  }\n'
